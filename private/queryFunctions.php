...
function getDaysDifference($new, $old)
{
    global $db;
    $sql = "SELECT DATEDIFF(" . $new['returnDate'] . ", " . $old['returnDate'] . ") as 'daysDiff'";
    $result = mysqli_query($db, $sql);
    confirm_result_set($result);
    $difference = mysqli_fetch_assoc($result);
    mysqli_free_result($result);
    return $difference['daysDiff'];
}
